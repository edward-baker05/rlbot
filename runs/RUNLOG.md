...
|---|---|---|---|---|
| 2026-08-24 | p19pool | **One concept, three coupled changes, declared.** Version pool curated to 5 opponents spanning 478 Elo (p12goal, p15manual, p16, p17, p18) replacing 32 near-clones spanning ~10; `trainAgainstOldChance` 0.2 -> 0.5; `tsPerVersion` 5M -> 200M. Seeded from `main-p18entropy/3075056128`, 400M steps to 3475M. Reward stack, obs, spawns, mask, LR, gamma, entropy, `tsPerItr` frozen | Item 11, the leading hypothesis since p17: 57.6% of reward mass has zero expected population advantage against a copy of itself, and 80% of training was exactly that | **CLEAN, WELL-POWERED NULL. 1 of 4 predictions, partial. Item 11 is FALSIFIED, not deferred.** Ladder (400 games/side/rung, in-distribution, SELF-control **50.1%** against a truth of 50%): **p19pool vs p18entropy head-to-head 49.9% goal share, CI [47.7%, 52.1%], = +10 Elo for 400M steps** against a ~+-13 noise floor and a +20 baseline expectation at the measured 5 Elo/100M. **The held-out rungs settle the exploitation question**: p13strike and p14aerial were NEVER in the training pool and moved +16 and +35 Elo, while pool members p12goal, p16 and p17 moved **-35, -9 and -5**. So there was no general improvement AND no specific exploitation of the pool either -- the policy simply did not move. Scorecard: **(1) `Rating/1v1` > 420 by +200M -- FAILED AND INVERTED**, seeded 394, peaked 388, ended **345** against a curated pool. **(2) zero-sum metrics leave their p18 bands -- PARTIAL**: `Shot/Saved Share` -5.2% (band 4.9%) and `Shot/Distance` -2.0% (band 1.6%) cleared, `Save/Converted` -0.3% did not. **(3) `SB3 Clip Fraction` > 0.040 -- FAILED**, 0.0366 -> 0.0374. **(4) more than +42 Elo over p16 -- FAILED**, +33 against p18's +42, i.e. slightly worse. **The finding that actually matters, and it is not the null.** `Average Step Reward` rose **+41.1%** and `Episode/Mean Steps` fell 5.6% -- not improvement, but the arithmetic consequence of spending half of training against opponents 304 and 478 Elo weaker. **Gradient was restored and the policy responded to it, and the response was worthless.** The k-arithmetic was right that a diverse pool creates expected advantage where a clone creates none; it was silent on the DIRECTION, and four of the five pool members were weaker than the challenger, two of them enormously. Optimising against opponents you already beat 89% of the time teaches nothing. **Restoring a gradient is not the same as restoring learning -- the signal has to point somewhere useful.** **What this does and does not license.** It falsifies "diversity toward WEAKER opponents helps", which is the only form of opponent diversity this project can generate from its own history. It does NOT test a stronger or equally-strong but *different* opponent, which is item 11's second limb (external-opponent Elo, never built; `libs/opponents/` holds Necto and Nexto and `scripts/match.sh` exists but has never run). **For v2: do not build opponent-diversity machinery out of your own past checkpoints. It is measurably worthless.** Also a standing measurement lesson: **when the opponent distribution changes, `Average Step Reward` stops being comparable across runs** |
