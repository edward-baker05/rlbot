...
fs::path FindLatestCheckpoint(const fs::path& runFolder) {
	std::error_code ec;
	fs::path targetDir = runFolder;
	if (!fs::is_directory(targetDir, ec)) {
		if (fs::is_directory(fs::path("bot/build") / runFolder, ec))
			targetDir = fs::path("bot/build") / runFolder;
		else if (fs::is_directory(fs::path("..") / runFolder, ec))
			targetDir = fs::path("..") / runFolder;
		else
			return {};
	}
