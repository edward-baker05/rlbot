...
// Imported: 2026-08-25
//
// These are NOT ours and are NOT from a trusted source (not Zealan/Rolv/Aech).
// They are kept as reference implementations and as a menu of options -- read one
// before you weight it into a run. Nothing here is used unless Rewards.cpp asks
// for it.
//
// The upstream pack advertises 56 rewards. What actually arrived:
//
//   39 imported here.
//   13 are STUBS -- `// TODO: Implement reward logic; return 0.0f` -- in BOTH the
...
// CradleFlickReward holds a CradleReward), so declaration order is load-bearing.
//
// Note the stubs are disproportionately the Control and Positioning entries, i.e.
// exactly the categories our own reward set is thinnest in. Boost discipline and
// rotation/spacing shaping remain unsolved by this import -- TeamSpacingReward
// and PositioningReward below are the only real positioning entries that landed.
//
...
//                                class; CommonValues' float 0/1 will not compare)
...
// SUPERSONIC_THRESHOLD, ...) come from RLGC::CommonValues via the using-directive
// below rather than being redefined, so there is one source of truth. The pack
// redefined them per-file with matching values; that was an obvious drift hazard.
//
// Weighting: per Zealan's rewards.md a reward should output within [-1, 1] so the
// weight alone determines its impact. Several of these do NOT respect that --
// TeamSpacingReward accumulates one penalty per teammate, so its range scales with
// team size. Check the range before you pick a weight.
...
// Contained on purpose: this is a vendored blob written against unqualified RLGC
// names, and qualifying every one of them by hand would make future re-imports
// from upstream a manual diff instead of a re-run of the importer.
...
// [ball]
	class BallYCoordinateReward : public Reward {
	    ...
	    public:
	        ...
	        virtual float GetReward(const Player& player, const GameState& state, bool isFinal) override {
	            float y = (player.team == Team::BLUE) ? state.ball.pos.y : -state.ball.pos.y;
...
// [positioning]
	class CenterReward : public Reward {
	    ...
	    public:
	        ...
	        virtual float GetReward(const Player& player, const GameState& state, bool isFinal) override {
	            Vec ball_loc = (player.team == Team::BLUE) ? state.ball.pos : -state.ball.pos;
	            ...
	            if (centered) {
	                ...
	                if (ball_dist_2d < centered_distance) {
	                    centered = true;
	                    reward += (state.lastTouchCarID == player.carId) ? 1.0f : non_participation_reward;
	                }
	            }
	            return reward;
	        }
...
// [positioning]
	class PositioningReward : public Reward {
	    ...
	    public:
	        virtual float GetReward(const Player& player, const GameState& state, bool isFinal) override {
	            Vec ball = (player.team == Team::BLUE) ? state.ball.pos : -state.ball.pos;
...
// [ball]
	class PushReward : public Reward {
	    ...
	    public:
	        virtual float GetReward(const Player& player, const GameState& state, bool isFinal) override {
	            Vec pos = (player.team == Team::BLUE) ? state.ball.pos : -state.ball.pos;
	            if (pos.y > 0.0f) {
...
// [velocity]
	class TweakedVelocityPlayerToGoalReward : public Reward {
	    ...
	    public:
	        ...
	        virtual float GetReward(const Player& player, const GameState& state, bool isFinal) override {
	            Vec ball_pos = (player.team == Team::BLUE) ? state.ball.pos : -state.ball.pos;
	            Vec player_pos = (player.team == Team::BLUE) ? player.pos : -player.pos;
	            Vec player_goal = (player.team == Team::BLUE) ? BLUE_GOAL_BACK : ORANGE_GOAL_BACK;
...
// [ball]
	class VelocityBallToGoalReward : public Reward {
	    public:
	        virtual float GetReward(const Player& player, const GameState& state, bool isFinal) override {
	            Vec ball_pos = (player.team == Team::BLUE) ? state.ball.pos : -state.ball.pos;
	            Vec goal_pos = (player.team == Team::BLUE) ? Vec(0.0f, 6000.0f, 321.3875f) : Vec(0.0f, -6000.0f, 321.3875f);
	                ...
	                if (challenged) {
	                    if (stable) {
	                        if (player.isOnGround) {
	                            ...
	                            return player.HasFlipOrJump() ? reward + 2.0f : reward + 5.0f;
	                        }
	                    }
	                } else if (stable) {
	                    return reward + 1.0f;
...
//
//   EventReward     -- superseded by RLGC::PlayerDataEventReward in CommonRewards.h,
//                      which does the same job natively off per-step `eventState`
//                      flags. The pack's version reconstructs cumulative match
//                      counters our Player does not carry (match_goals /
//                      match_shots / match_saves / match_demolishes) and reads team
//                      scores our GameState does not carry either. It also indexes
//                      `last_registered_values[player.carId]` into a vector sized by
//                      player COUNT -- carId is an id, not an index, so that is an
//                      out-of-bounds read waiting to happen. Use
//                      PlayerDataEventReward instead.
//
//   DefenderReward  -- needs a running scoreline to detect a concede. Our GameState
//                      exposes only `goalScored` (a per-step bool) and no score, so
//                      a faithful port would have to infer the scoring team from
//                      ball position at the goal tick and keep its own counter.
//                      That is a rewrite, not an import, and it would inherit the
//                      original's flaw of sharing one `enemy_goals` counter across
//                      every player in the arena. Left out rather than half-ported.
