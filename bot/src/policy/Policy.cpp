...
Policy::Policy(ObsBuilder *obsBuilder, int obsSize, ActionParser *actionParser,
			   const ModelShape &shape, bool useGPU)
...
void Policy::Load(const std::filesystem::path &folder) {
	if (!std::filesystem::is_directory(folder)) {
		throw std::runtime_error(
			std::string("Policy: checkpoint folder does not exist: ") +
			folder.string());
	}
	...
	if (!anyModelFile) {
		throw std::runtime_error(std::string("Policy: no .lt model files in ") +
								 folder.string() +
								 " (expected a GigaLearn checkpoint folder, "
...
std::vector<Action> Policy::InferBatch(const std::vector<Player> &players,
	...
	if (players.size() != states.size())
		throw std::runtime_error(
			"Policy::InferBatch(): mismatched input sizes");
	if (!unit)
