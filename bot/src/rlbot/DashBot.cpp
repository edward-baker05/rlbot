...
static int EnvIntOr(const char *key, int fallback) {
	...
	try {
		...
		std::fprintf(stderr,
					 "[DashBot] WARNING: %s is not an integer, using %d\n", key,
					 fallback);
		return fallback;
	}
}
...
BotSettings BotSettings::FromEnvironment() {
	...
	s.collisionMeshes =
		EnvOr("DASH_COLLISION_MESHES",
			  EnvOr("HIVE_COLLISION_MESHES", "collision_meshes"));
	s.maxPlayersPerTeam = EnvIntOr("DASH_MAX_PLAYERS_PER_TEAM",
								   EnvIntOr("HIVE_MAX_PLAYERS_PER_TEAM", 3));
	s.tickSkip = EnvIntOr("DASH_TICK_SKIP", EnvIntOr("HIVE_TICK_SKIP", 8));
	s.maskActions =
		EnvIntOr("DASH_MASK_ACTIONS", EnvIntOr("HIVE_MASK_ACTIONS", 0)) != 0;
	...
	s.actionDelay =
		EnvIntOr("DASH_ACTION_DELAY", EnvIntOr("HIVE_ACTION_DELAY", 7));
	s.deterministic =
		EnvIntOr("DASH_DETERMINISTIC", EnvIntOr("HIVE_DETERMINISTIC", 1)) != 0;
...
void SharedContext::Initialize(const BotSettings &s) {
	...
	policy =
		std::make_unique<Policy>(obsBuilder.get(), obsSize, actionParser.get(),
								 settings.modelShape, settings.useGPU);

	policy->Load(settings.model);
	std::printf("[DashBot] Loaded model from %s\n", settings.model.c_str());

	std::printf(
		"[DashBot] Observation size %d, tickSkip %d, actionDelay %d, %s\n",
		obsSize, settings.tickSkip, settings.actionDelay,
		settings.useGPU ? "GPU" : "CPU");
}

DashBot::DashBot(std::unordered_set<unsigned> indices, unsigned team,
				 std::string name) noexcept
	: rlbot::Bot(std::move(indices), team, std::move(name)) {
	std::printf("[DashBot] Team %u controlling %zu car(s):", team,
				this->indices.size());
...
void DashBot::initialize(
	const rlbot::flat::ControllableTeamInfo *controllableTeamInfo,
...
void DashBot::update(
	const rlbot::flat::GamePacket *packet,
	...
	try {
		...
		const float now = packet->match_info()
							  ? packet->match_info()->seconds_elapsed()
							  : 0.f;
		int ticksElapsed = 1;
		if (prevSeconds >= 0.f) {
			ticksElapsed =
				static_cast<int>(std::lround((now - prevSeconds) * 120.f));
		...
		for (unsigned idx : indices) {
			...
			setOutput(idx,
					  rlbot::flat::ControllerState(
						  car.applied.throttle, car.applied.steer,
						  car.applied.pitch, car.applied.yaw, car.applied.roll,
						  car.applied.jump == 1.f, car.applied.boost == 1.f,
						  car.applied.handbrake == 1.f, false));
		}
		...
		if (!loggedError) {
			loggedError = true;
			std::fprintf(stderr,
						 "[DashBot] ERROR in update(): unknown exception\n");
		}
		for (unsigned idx : indices)
			setOutput(idx, {});
	}
